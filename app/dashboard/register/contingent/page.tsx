...
export default function ContingentRegisterPage() {
  ...
  // Step 0: institution details, captured once.
  ...
  // Steps 1..N: one roster per event.
  const [rosters, setRosters] = React.useState<Record<string, TeamMemberInfo[]>>({});
  const [eventIndex, setEventIndex] = React.useState(0); // 0 = intro/institution step
  ...
  const institutionComplete = Boolean(
    collegeName.trim() && city.trim() && facultyName.trim() && facultyEmail.trim() && facultyPhone.trim()
  );
  ...
  async function submitContingent() {
    ...
    try {
      ...
      const res = await fetch("/api/v1/registrations/contingent", {
        method: "POST",
      ...
      setRegisteredCount(json.data.length);
      setPhase("payment");
    } catch {
  ...
  if (phase === "loading") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-sm text-muted-foreground">Loading contingent registry...</p>
      </div>
    );
  }
      ...
      <div className="max-w-xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {`One flow to register your college's delegation across all ${events.length} events. Enter your college details once — you'll pick each event's team next, and pay once at the end.`}
          </p>
        </div>
        <Card className="glass border-white/10">
          <CardContent className="p-6 space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input id="college-name" placeholder="e.g. IIM Bangalore" value={collegeName} onChange={(e) => setCollegeName(e.target.value)} required />
              </div>
              <div className="space-y-1">
                <Label htmlFor="college-city">City</Label>
                <Input id="college-city" placeholder="e.g. Bengaluru" value={city} onChange={(e) => setCity(e.target.value)} required />
              </div>
            </div>
            <div className="pt-2 border-t border-white/5 space-y-2">
              <p className="text-xs font-bold uppercase tracking-wider text-amber-400">Faculty Coordinator</p>
              <div className="grid sm:grid-cols-3 gap-2">
                <Input placeholder="Faculty Name" value={facultyName} onChange={(e) => setFacultyName(e.target.value)} required />
                <Input placeholder="Faculty Email" type="email" value={facultyEmail} onChange={(e) => setFacultyEmail(e.target.value)} required />
                <Input placeholder="Faculty Phone" value={facultyPhone} onChange={(e) => setFacultyPhone(e.target.value)} required />
              </div>
            </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex gap-2 pt-2">
        <Button
          className="flex-1"
          disabled={!currentEventComplete || submittingReg}
          onClick={goNext}
        >
