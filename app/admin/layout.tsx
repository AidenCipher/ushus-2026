    ...
    <div className="flex min-h-screen bg-background relative overflow-hidden">
      ...
      >
        <div className="flex h-full flex-col">
          {/* Logo Area */}
          <div className="h-16 flex items-center px-6 border-b border-rose-500/20 bg-rose-950/10">
            <Link href="/" className="flex items-center gap-2 group">
              <div className="w-8 h-8 rounded-lg bg-rose-500/20 flex items-center justify-center border border-rose-500/50 shadow-[0_0_10px_rgba(244,63,94,0.3)]">
                <TerminalSquare className="w-4 h-4 text-rose-500" />
              </div>
              <div className="flex flex-col">
                <span className="font-bold tracking-tight leading-tight text-rose-50">ADMIN</span>
                <span className="text-[10px] text-rose-500 font-medium uppercase tracking-wider">Superuser</span>
              </div>
            </Link>
          </div>
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? "bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-[inset_2px_0_0_rgba(244,63,94,1)]" 
                      : "text-muted-foreground hover:bg-white/5 hover:text-foreground"
                  )}
                >
                  <link.icon className={cn("w-5 h-5", isActive ? "text-rose-400" : "text-muted-foreground group-hover:text-foreground")} />
                  {link.name}
                </Link>
              );
            })}
          ...
          {/* Bottom Actions */}
          <div className="p-4 border-t border-white/10 bg-rose-950/5">
            <Button 
              variant="ghost" 
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header */}
        <header className="h-16 flex items-center justify-between px-4 lg:px-8 border-b border-rose-500/20 bg-background/50 backdrop-blur-md sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold capitalize hidden sm:block">
              {pathname.split("/").pop() === "admin" ? "System Configuration" : pathname.split("/").pop()}
            </h1>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4 lg:p-8 relative">
           <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden opacity-30">
             <div className="absolute top-0 right-0 w-96 h-96 bg-rose-500/10 rounded-full filter blur-[100px]" />
           </div>
